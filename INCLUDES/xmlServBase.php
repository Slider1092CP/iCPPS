...
abstract class xmlServBase{
	...
	public function initialise(){
		if (($this->mainSocket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)) < 0){
			$this->log->error("socket_create() failed: " . socket_strerror($this->mainSocket));
			return false;
		}
		if (!socket_set_option($this->mainSocket, SOL_SOCKET, SO_REUSEADDR, 1)) {
			$this->log->log("Fatal error: " . socket_strerror(socket_last_error($this->mainSocket)));
			exit;
		}
	/*	if (!socket_set_option($this->mainSocket, SOL_SOCKET, SO_LINGER, 4)) {//Wait 4 seconds before closing a socket(After close)
			$this->log->log("Fatal error: " . socket_strerror(socket_last_error($this->mainSocket)));
			exit;
		}*/
		if (($ret = socket_bind($this->mainSocket, $this->config["ADDRESS"], $this->config["PORT"])) < 0) {
			$this->log->error("socket_bind() failed: " . socket_strerror($ret));
			return false;
		}
		if (($ret = socket_listen($this->mainSocket, 5)) < 0) {
			$this->log->error("socket_listen() failed: " . socket_strerror($ret));
			return false;
		}
	...
	function parseData($str, $clientid){
		...
		if(stripos($str, "policy-file-request") !== false){
			$this->sendPolicyFile($this->clients[$clientid]);
		}
		if($this->config["RAW_STR"] && $str[0] == $this->config["RAW_SEPERATOR"])
			return $this->parseRawData($str, $clientid);
		...
		if(!$called){
			$this->unknownHandler($data, $str, $clientid);
		}
		return 0;
	}
	...
	function getSockets($clients){
		...
		foreach($clients as $key => $client){
			$sockets[$key + 1] = $client->getSocket();
		}
		$isn = false;
