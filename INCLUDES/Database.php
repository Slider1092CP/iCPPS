...
function &getDB(){
	...
	//$g_link = mysql_connect( 'localhost', 'root', 'E9hfAZLCvU5W') or die('Could not connect to server.' );
	//mysql_select_db('penguins', $g_link) or die('Could not select database.');
	return $g_link;
}
...
function getPlayer($id){
	...
	if(!is_array($a))
		return BAD_USER;
	$s = $id;
	$s .= "|" . getName($id) . "|" . 1;//ENGLISH BITMASK!
	$s .= "|" . $a["color"];
	$s .= "|" . $a["head"];
	$s .= "|" . $a["face"];
	$s .= "|" . $a["neck"];
